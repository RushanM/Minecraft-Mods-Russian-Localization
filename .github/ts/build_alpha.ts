...
(async () => {
  try {
    ...
    console.log(
      "Содержание каталога «Набор ресурсов»:",
      fs.readdirSync("Набор ресурсов")
    );
...
function getChangedFiles(lastTag: string | null): ChangedFile[] {
  ...
  let diffCommand =
    "git -c core.quotepath=false -c i18n.logOutputEncoding=UTF-8 diff --name-status";
  if (lastTag) {
...
async function getModChanges(
  ...
  for (const file of changedFiles) {
    ...
    const packMcmetaMatch = decodedFilePath.match(
      /^Набор ресурсов\/([^/]+)\/pack\.mcmeta$/
    );
    ...
    const langOrBookMatch = decodedFilePath.match(
      /^Набор ресурсов\/([^/]+)\/assets\/([^/]+)\/(lang|book)\/.+\.(json|lang|txt|xml)$/i
    );
    if (langOrBookMatch) {
...
async function getModInfoFromSheet(
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const rowNameRaw = row[nameIndex]
      ? row[nameIndex].trim().toLowerCase()
      : "";
    // Удаление пробелов, апострофов из сырого названия строки
    const rowNameNormalized = rowNameRaw.replace(/[ '\']/g, "");
    const rowGameVerRaw = row[gameVerIndex]
      ? row[gameVerIndex].trim().toLowerCase()
      : "";
...
async function generateReleaseNotes(
  ...
  const { modChanges, newGameVersions } = await getModChanges(
    changedFiles,
    sheets
  );
  ...
  for (const group of groupedList) {
    ...
    if (versions.length === 1) {
      ...
      if (start === end) {
        versionText = start === "b1.7.3" ? start : `${start}.x`;
      } else {
...
function createArchives(
  ...
  if (fs.existsSync(resourcePackDir)) {
    const resourcePackVersions = fs
      ...
      .filter((ver) =>
        fs.statSync(path.join(resourcePackDir, ver)).isDirectory()
      );
    ...
    resourcePackVersions.forEach((ver) => {
      ...
      const relatedFiles = changedFiles.filter((file) =>
        file.filePath.startsWith(versionDir)
      );
      ...
      ["pack.mcmeta", "dynamicmcpack.json", "respackopts.json5"].forEach(
        (fileName) => {
  ...
  if (fs.existsSync(shaderPacksDir)) {
    const shaderPacks = fs
      ...
      .filter((pack) =>
        fs.statSync(path.join(shaderPacksDir, pack)).isDirectory()
      );
    ...
    shaderPacks.forEach((pack) => {
      ...
      const relatedFiles = changedFiles.filter((file) =>
        file.filePath.startsWith(packDir)
      );
  ...
  if (fs.existsSync(modpacksDir)) {
    const modpacks = fs.readdirSync(modpacksDir).filter((modpack) => {
      ...
      return (
        ...
        fs.statSync(translationPath).isDirectory()
      );
    });
